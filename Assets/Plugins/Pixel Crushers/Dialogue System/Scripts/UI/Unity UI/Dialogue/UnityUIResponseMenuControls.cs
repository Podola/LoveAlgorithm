                ...
                if ((buttonTemplate != null) && (buttonTemplateHolder != null))
                {
                    ...
                    for (int i = 0; i < responses.Length; i++)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                Debug.LogError(string.Format("{0}: Response button template doesn't have a UnityUIResponseButton component", DialogueDebug.Prefix));
                                continue;
                            }
                            SetResponseButton(responseButton, responses[i], target, buttonNumber++);
                            // Ensure button's onClick is properly connected
                            if (responseButton.button != null)
                            {
                                responseButton.button.onClick.RemoveAllListeners();
                                responseButton.button.onClick.AddListener(() => {
                                    if (responseButton != null && responseButton.response != null)
                                    {
                            ...
                            {
                                Debug.LogError(string.Format("{0}: Response button '{1}' doesn't have a Button component", DialogueDebug.Prefix, buttonGameObject.name));
                            }
